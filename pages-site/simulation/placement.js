...
export function createPlacement({scene,camera,canvas,map,models,actors,draw,isLocked}){
 ...
 let selected=null,ghost=null,hover=null,effectFrame=0;const effects=[],reduced=matchMedia('(prefers-reduced-motion: reduce)');
 ...
 function finishEffects(){cancelAnimationFrame(effectFrame);effectFrame=0;for(const e of effects){e.actor.scale.setScalar(1);e.actor.position.y=e.y;scene.remove(e.pulse);e.pulse.material.dispose();}effects.length=0;}
 function animate(now){effectFrame=0;for(let i=effects.length-1;i>=0;i--){const e=effects[i],t=Math.min(1,(now-e.start)/700),settle=1-Math.pow(1-t,3);e.actor.position.y=e.y+5*(1-settle);e.actor.scale.setScalar(t<.7?.75+.25*settle:1+Math.sin((t-.7)/.3*Math.PI)*.06);e.pulse.scale.setScalar(1+t*2.5);e.pulse.material.opacity=(1-t)*.8;
   if(t===1){e.actor.position.y=e.y;e.actor.scale.setScalar(1);scene.remove(e.pulse);e.pulse.material.dispose();effects.splice(i,1);}}
  draw();if(effects.length)effectFrame=requestAnimationFrame(animate);
 }
 ...
 document.querySelector('#placement-undo').addEventListener('click',()=>{if(isLocked())return;finishEffects();actors.undoTower();save();status.textContent='Last added tower removed.';hide();});
 document.querySelector('#placement-clear').addEventListener('click',()=>{if(isLocked())return;finishEffects();actors.clearTowers();save();status.textContent='Added towers cleared. Preset towers remain.';hide();});
 ...
 return {pointer,click,hide,select,placeAt,validAt,screenPoint,prepareCapture,finishEffects,refresh:()=>{ui();if(!isLocked())status.textContent='Select a tower to keep building. Your added towers stay in place.';},state:()=>({selected,hover,animating:effects.length,locked:isLocked()})};
}
