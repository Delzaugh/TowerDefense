...
const ids = ['campus_coffee_kiosk','campus_planter_bench','campus_wayfinding_sign','campus_meeting_nook','campus_code_sculpture','campus_pixel_copilot'];
(async()=>{
  ...
  try {
    ...
    const output=path.join(project,'artifacts/campus-decor');await fs.mkdir(output,{recursive:true});
    ...
    await sharp({create:{width:1350,height:876,channels:4,background:'#172531'}}).composite(overview).png().toFile(path.join(output,'overview.png'));
