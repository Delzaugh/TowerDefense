# Linter Agent — detailed specification

![Linter Agent sheet](linter_agent_spec_v01.png)
...
Short-range omnidirectional defense that launches radial volleys without selecting an individual target.

| Property | Initial playtest design |
| --- | --- |
| Base + Persona | 30 + 20 = 50 Compute |
| Range | 3 map units |
| Action interval | 0.5 s per volley |
| Projectiles per volley | 8 at 45-degree spacing |
| Damage | 2 per connecting projectile |
| Work output | None |
| Footprint radius | 0.4 logical map units |

Rule Burst fires eight straight, non-homing projectiles when an eligible Enemy is in effective range. Each projectile damages one Enemy once, then disappears; obstacles and range limits stop it. Actual output depends on connections, not a guaranteed 32 damage/sec.
...
1. Flattened octagonal shell is the body, with a broad charcoal top inset and a low graphite lower rim.
2. Build one clean square emitter module and duplicate at eight equal angular steps around the perimeter.
   Proposed visual clocking is 22.5 + 45n degrees from the front centerline (n = 0 through 7), placing a pair on either side of both front and rear centerlines. Confirm exact module spacing in the actual mesh; the concept images are illustrative. This visual clocking does not prescribe simulation projectile orientation.
3. Front goggle strip and small dark face are integrated into the front facet without blocking emitters.
4. Rear facets have emitters but no repeated facial features; silhouette should remain a compact rule-sweeping device.
...
- Swatch 1: Lime shell.
- Swatch 2: Charcoal top.
- Swatch 3: Dark graphite rim.
- Swatch 4: Off-white edge.
- Swatch 5: Cyan eyes.
...
Proposed: compact energetic hover and a short radial pulse/settle to present a volley. Avoid continuous shell rotation unless front-face orientation is intentionally addressed in a later rig design. Projectiles and their collisions are runtime-owned.
...
- Generated with the built-in image generation tool using the model-spec-sheet template; prompt saved in prompts/linter_agent.txt.
