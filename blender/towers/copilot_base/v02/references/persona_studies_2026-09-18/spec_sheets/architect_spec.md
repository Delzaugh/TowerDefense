...
![Architect sheet](architect_spec_v01.png)
...
Enemy-only long-range area bombardment against groups.

| Property | Initial playtest design |
| --- | --- |
| Base + Persona | 30 + 30 = 60 Compute |
| Range | 8 map units |
| Action interval | 1.2 s |
| Explosion radius | 1.5 map units |
| Damage | 16 per eligible Enemy in explosion |
| Work output | None |
| Auto target commitment | 0.5 s, Enemies only |
| Footprint radius | 0.4 logical map units |

Structural Impact deals 16 damage to each eligible Enemy within 1.5 units of impact, with no initial falloff. Blockage is checked from impact. Architect never targets productive Work and has no Technical Debt cleanup bonus.
...
1. A wide rectangular teal body supports an integrated ivory structural arch; continuity and visible load paths matter.
2. One upward-tilted square cyan aperture is the action origin; avoid the silhouette of a gun barrel.
3. Broad engineering goggles, lower face window, large side joints and stepped buttresses retain the heavy designed-for-load appearance.
4. Back two-slot teal panel sits inside the same ivory frame rather than adding a separate backpack.
...
- Swatch 1: Petrol-teal shell.
- Swatch 2: Stone ivory.
- Swatch 3: Brass side joints.
- Swatch 4: Navy face.
- Swatch 5: Cyan projector.
...
Proposed: slow confident hover; short projector preparation tilt and small body settle when presenting a shot. No root translation, projectile spawn timing or impact outcome encoded in a clip.
...
- Generated with the built-in image generation tool using the model-spec-sheet template; prompt saved in prompts/architect.txt.
