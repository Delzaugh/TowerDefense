...
# Broad, quiet facets and a narrow, irregular dorsal separation.
profiles=[(-.57,.36,1.00,.51,0),(-.37,.64,1.30,.48,0),(.02,.79,1.47,.47,.027),(.43,.76,1.42,.48,-.02),(.78,.53,1.18,.51,.018),(.97,.12,.80,.57,0)]
for side in [-1,1]:
    vs=[]
    for y,w,top,low,jitter in profiles:
        for j in range(5):
            t=math.pi*.5*j/4
            vs.append((jitter+side*(.04+(w-.04)*math.sin(t)),y,low+(top-low)*math.cos(t)))
    fs=[]
    for k in range(len(profiles)-1):
        for j in range(4):
            a=k*5+j;b=a+5
            # Large triangulated planes, alternating diagonals.
            fs.extend([(a,b,b+1),(a,b+1,a+1)])
    fs.extend([tuple(reversed(range(5))),tuple(range(25,30)),tuple([i*5 for i in range(6)]+[i*5+4 for i in reversed(range(6))])])
    mesh(vs,fs,'shell')
    # Rolled carapace edge: raised bevel, dark undercut, and clean end closures.
    rim=[]
    for y,w,top,low,jitter in profiles:
        x3=.04+(w-.04)*math.sin(math.pi*3/8)
        rim.extend([(jitter+side*(x3*.18+w*.82+.004),y,low+(top-low)*math.cos(math.pi*3/8)*.18),
                    (jitter+side*(w+.009),y,low+.006),
                    (jitter+side*(w-.023),y,low-.035)])
    rf=[];rc=[]
    for k in range(5):
        for j in range(2):rf.append((k*3+j,(k+1)*3+j,(k+1)*3+j+1,k*3+j+1));rc.append('shell_dark' if j==0 else 'edge')
    rf.extend([(0,1,2),(15,17,16)]);rc.extend(['edge','edge']);mesh(rim,rf,rc)
vs=[]
for y,w,top,low,j in profiles:vs.extend([(j-.026,y,top-.025),(j+.026,y,top-.025)])
mesh(vs,[(i*2,i*2+1,i*2+3,i*2+2) for i in range(5)],'red',glow=.15)
ellipsoid((0,.17,.53),(.70,.77,.27),'edge',segments=12,rings=5)
