...
const ids = ["codicon_agent","codicon_book","codicon_lightbulb","codicon_mcp","codicon_github","codicon_code","codicon_copilot"];
(async()=>{
  ...
  try {
    for(const [i,id] of ids.entries()){
      ...
      const label=Buffer.from('<svg width="340" height="38"><rect width="340" height="38" fill="#172531"/><text x="18" y="25" fill="#e2edf0" font-family="sans-serif" font-size="17">'+id.replace('codicon_','').replaceAll('_',' ')+'</text></svg>');
      overview.push({input:await sharp(path.join(dir,'validation/iso.png')).resize(340,300).png().toBuffer(),left:(i%4)*340,top:Math.floor(i/4)*338});
      overview.push({input:label,left:(i%4)*340,top:Math.floor(i/4)*338+300});
    }
    if(errors.length)throw Error(errors.join('\n'));
    const output=path.join(project,'artifacts/codicons');await fs.mkdir(output,{recursive:true});
    ...
    await sharp({create:{width:1360,height:Math.ceil(ids.length/4)*338,channels:4,background:'#172531'}}).composite(overview).png().toFile(path.join(output,'overview.png'));
