# Tester — detailed specification

![Tester sheet](tester_spec_v01.png)
...
Base-level direct output with continuous quality support inside its action coverage.
...
QA Aura slows Enemies by 10% while in coverage and adds +1 Compute per productive Work item completed within coverage, regardless of finisher. Overlaps use the strongest slow and bonus separately; no stacking.
...
1. Rounded mint shell with broad ivory cheeks and lower band; seam follows a consistent joint around the rear.
2. Large scalloped focus ring on character-left and smaller circular lens on character-right; preserve this handedness through all views.
3. Side calibration bracket marks and one front check badge carry the QA identity; keep symbols broad.
...
Proposed: curious small head tilts, subtle focus-ring adjustment and focused work nod. QA coverage and slow/reward feedback are runtime effects, active independently of the work clip.
...
- Generated with the built-in image generation tool using the model-spec-sheet template; prompt saved in prompts/tester.txt.
